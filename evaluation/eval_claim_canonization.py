...
# runs each of the six benchmark LLMs (bf16) through the canonization prompt.
#
...
# Output
# ------
#   • A per-claim output table showing all 6 models' canonizations.
#   • An overall statistics table (English rate + latency).
...
def eval_claim_canonization_benchmark(project_root: Path, console=None) -> None:
    """Benchmark the six-model catalogue (bf16) on multilingual claim canonization.
    ...
    Output
    ------
    - A per-claim Rich table with each model's output for all 6 models.
    - An overall statistics table (English rate, latency).
    ...
    # all_results[model_key] = list[dict]
    ...
    for model_key in BENCH_MODEL_KEYS:
        ...
        _print(f"[dim]Loading {display_name} (bf16)…[/dim]")
    ...
    tbl = Table(
        ...
        show_lines   = True,
        title        = "Claim Canonization Outputs (bf16)",
    )
    ...
    summary_tbl = Table(
        title        = f"Claim Canonization Benchmark — {len(BENCH_MODEL_KEYS)} models (bf16)",
        border_style = "blue",
        box          = _box.ROUNDED,
    )
